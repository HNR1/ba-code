...
The experiments we conducted suggest that ToMe's performance can be improved by applying token merging in both the self-attn and cross-attn layer of the transformer. Furthermore, the best performance can be achieved by using $1 \times 2$ for token partitioning, bringing particularly great improvements to image quality when \(r\leq40\%\).\\

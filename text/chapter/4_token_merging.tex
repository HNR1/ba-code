...
The tokens are partitioned into a source (\textbf{src}) and a destination (\textbf{dst}) set (see Fig.~\ref{fig:tome}), then the most similar tokens from the \textbf{src} set are continuously merged into their \textbf{dst} counterparts until the number of tokens has reduced by \(r\)\%.\\
...
{static/src_dst.png}
\caption{Transformer with ToMe and partition of tokens into src (red) and dst (blue). \cite{bolya2023tomesd}.}
\label{fig:tome}
\end{figure}
